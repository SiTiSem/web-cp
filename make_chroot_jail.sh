...
#
# 1/21/2016 - Jon M, Fixed to work with i386 32-bit Debian Jessie. It 
#             might work with amd64.
#####################################################################
...
if [ "$DISTRO" = SUSE ]; then
  ...
  APPS="/bin/bash /bin/cp /usr/bin/dircolors /bin/ls /bin/mkdir /bin/mv /bin/rm /bin/rmdir /bin/sh /bin/su /usr/bin/groups /usr/bin/id /usr/bin/rsync /usr/bin/ssh /usr/bin/scp /sbin/unix_chkpwd /bin/nano /bin/tar /usr/bin/wget /usr/bin/zip /usr/bin/unzip /usr/bin/touch /usr/bin/vi /usr/bin/vim.tiny /usr/bin/find"
else
...
JAILDIRS="dev etc etc/pam.d bin www sbin usr usr/bin usr/lib"
...
#HOMEDIR="$JAILPATH/www/$CHROOT_USERNAME"
HOMEDIR="/"
...
#
TMPFILE1=`mktemp` ||  TMPFILE1="${HOME}/ldlist"
if [ -x ${TMPFILE1} ]; then 
  mv ${TMPFILE1} ${TMPFILE1}.bak
fi
TMPFILE2=`mktemp` || TMPFILE2="${HOME}/ldlist2"
if [ -x ${TMPFILE2} ]; then 
  mv ${TMPFILE2} ${TMPFILE2}.bak
fi
...
if [ "$DISTRO" = SUSE ]; then
  ...
  if [ -d /lib/x86_64-linux-gnu ]; then
    OTHERLIBPATH="lib/x86_64-linux-gnu"
  fi
  if [ -d /lib/i386-linux-gnu ]; then
    OTHERLIBPATH="lib/i386-linux-gnu"
  fi
  if [ "$OTHERLIBPATH" != "" ]; then
    if ! [ -d ${JAILPATH}/${OTHERLIBPATH} ]; then
      mkdir -p ${JAILPATH}/${OTHERLIBPATH}
    fi
    cp /${OTHERLIBPATH}/libnss_compat.so.2 /${OTHERLIBPATH}/libnsl.so.1 /${OTHERLIBPATH}/libnss_files.so.2 \
      /${OTHERLIBPATH}/libcap.so.2 /${OTHERLIBPATH}/libnss_dns.so.2 ${JAILPATH}/${OTHERLIBPATH}
  fi
  else
...
if [ "$DISTRO" = DEBIAN ]; then
  ...
  if [ "$OTHERLIBPATH" != "" ]; then
    cp -r /${OTHERLIBPATH}/security ${JAILPATH}/${OTHERLIBPATH}
  fi
  else
